...
class CIFAR10LocalFileReader(numRecords: Int) extends FileCallback {
  ...
  override def call[T](file: File): T = {
    ...
    val ndarr =  Nd4j.create(arr.map(_.toDouble )).reshape( numRecords, RECORD_SIZE)

    val samples = ndarr(->, 1->).reshape(numRecords, NUM_CHANNELS, IMG_HEIGHT, IMG_WIDTH) / 255 - 0.5
